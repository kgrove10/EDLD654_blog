...
In this post, we will outline how we fit a boosted tree model to our data. Most of our information on boosted trees comes from our lovely Machine Learning instructors [Daniel Anderson and Joe Nese](https://uo-datasci-specialization.github.io/c4-ml-fall-2020/)
...
A description of our data prep procedures can be found [here](https://edld654finalproject.netlify.app/2020/11/30/data-description/). The model fit was completed using the University of Oregon's "Talapas" Supercomputer, so while some example code will be included with this post, computations were conducted elsewhere, and the full code can be found on our [GitHub here](https://github.com/kgrove10/EDLD654_blog/blob/main/static/boosted_forest_model). We will start out with a default Boosted Tree Model with no hyperparameters. The arguments defined are based on recommendations from Daniel Anderson and Joe Nese. 
...
What if we really wanted to try and reduce the rmse by tunung the hyperparameters?
...
bt_grid_metrics %>%
  write.csv("./BTTuneMetricsGrid_Tuned.csv", row.names = FALSE)
...
tunedres <- import(here("static", "boosted_forest_model", "BTTuneMetricsGrid_Tuned.csv"))

knitr::kable(tunedres)
```

It looks like the best model is XXX. Let's create a plot using the following code: 
...
According to this visualization, it looks like XX is the best learning rate

```{r viz, echo=FALSE}
blogdown::shortcode("figure", src ='"/boosted_forest_model/BTMetricsGrid_Tuned.pdf"', alt = "Tuned Metrics Visualization")
