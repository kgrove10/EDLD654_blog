...
This is in line with the general guidelines for these predictors: `mtry` between 2 and `p` (representing the number of predictors in the data), `trees` at least 1000, and `min_n` starting between 1 and 10 and adjust depending on impact to accuracy and run time. Here, our `min_n` is much larger, but as we can see from the visualization, our metrics argely improve as the value of `min_n` increases. In fact, we could consider increasing beyond the tuned value of 38, since it still appears to be decreasing, but in the interest of computational expense, we will stick with this selected value of 38. 
