...
Finally, the code below describes how we then generated score predictions for the testing set, `test.csv` based on the tuned random forest model, using the final workflow created above. Note that for computational efficiency, since the other models had to be run on Talapas, we generated these predictions for each of our models, but only submitted the predictions for the testing set on Kaggle that corresponded to the model with the best rmse based on our validation set. For more information, see our [final fit post](https://edld654finalproject.netlify.app/2020/12/05/final-model-fit/).   
